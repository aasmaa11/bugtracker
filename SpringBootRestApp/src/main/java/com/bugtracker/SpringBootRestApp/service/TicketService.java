...
public class TicketService {
	...
	@Transactional
	public List<Ticket> getAllTicketsForDeveloper(Long developerId) {
	...
	@Transactional
	public List<Ticket> getAllTicketsForSubmitter(Long submitterId) {
	...
	public List<TicketHistory> getAllTicketHistoryForTicket(int ticketId) {
		Ticket ticket = ticketRepository.findById(ticketId);
		List<TicketHistory> ticketHistories = toList(ticket.getTicketHistories());

		// Sort comments by date of change
		Collections.sort(ticketHistories, new Comparator<TicketHistory>() {
			public int compare(TicketHistory t1, TicketHistory t2) {
				return t2.getDateOfChange().compareTo(t1.getDateOfChange());
			}
		});
	...
	@Transactional
	public Ticket createTicket(String title, String description, Ticket.TicketPriority priority,
			Ticket.TicketStatus status, Ticket.TicketType type, Set<TicketAttachment> ticketAttachments,
			Set<Developer> developers, Set<Comment> comments, Integer projectId, String submitterUsername) {
		String error = "";
		...
		if (submitterUsername == null || submitterUsername.isEmpty()) {
	...
	@Transactional
	public Ticket setProject(Integer id, Integer projectId) {
	...
	@Transactional
	public Ticket addTicketAttachment(Integer ticketId, String file, String notes, String creatorUsername) {
		String error = "";
		if (file == null) {
	...
	@Transactional
	public Ticket addTicketHistory(Integer ticketId, String propertyChanged, String oldValueOfProperty,
	...
	@Transactional
	public Ticket createComment(String message, Integer ticketId, String commenterUsername) {
		String error = "";
		if (message == null) {
	...
	@Transactional
	public Ticket addAssignedDeveloper(Integer ticketId, String developerUsername) {
	...
	@Transactional
	public Ticket modifyTicket(Integer id, String title, String description, Ticket.TicketPriority priority,
			Ticket.TicketStatus status, Ticket.TicketType type) {
		if (!ticketRepository.existsById(id)) {
			throw new IllegalArgumentException("Ticket does not exist!");
		}
		String error = "";
		...
		if (!ticket.getTitle().equals(title)) {
			addTicketHistory(id, "title", ticket.getTitle(), title);
			ticket.setTitle(title);
		}

		if (!ticket.getDescription().equals(description)) {
			addTicketHistory(id, "description", ticket.getDescription(), description);
			ticket.setDescription(description);
		}
		if (!ticket.getPriority().equals(priority)) {
			addTicketHistory(id, "priority", ticket.getPriority().toString(), priority.toString());
			ticket.setPriority(priority);
		}
		if (!ticket.getStatus().equals(status)) {
			addTicketHistory(id, "status", ticket.getStatus().toString(), status.toString());
			ticket.setStatus(status);
		}
		if (!ticket.getType().equals(type)) {
			addTicketHistory(id, "type", ticket.getType().toString(), type.toString());
			ticket.setType(type);
		}
		ticketRepository.save(ticket);
	...
	@Transactional
	public Ticket setAssignedDevelopers(Integer ticketId, List<String> usernames) {
		if (!ticketRepository.existsById(ticketId)) {
			throw new IllegalArgumentException("Ticket does not exist!");
		}
		Ticket ticket = ticketRepository.findById(ticketId);
	...
	@Transactional
	public Comment modifyComment(String message, Integer commentId) {
		String error = "";
		if (commentId == null) {
